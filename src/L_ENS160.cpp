...
L_ENS160::L_ENS160(TwoWire &wirePort)
{
  ...
  _address = ENS160_I2C_ADDRESS; // Asigna la dirección I2C del sensor
}
...
bool L_ENS160::begin()
{
  ...
  _i2cPort->write(ENS160_OPMODE);        // Escribe la dirección del registro OPMODE
  _i2cPort->write(ENS160_MODE_STANDARD); // Escribe el valor para modo STANDARD
  _i2cPort->endTransmission();           // Finaliza la comunicación

  uint8_t status = readRegister(ENS160_PART_ID); // Lee el registro de indentificación del sensor
...
uint16_t L_ENS160::readCO2()
{
  ...
  readRegisters(ENS160_ECO2, data, 2); // Supuesto registro del CO2
...
void L_ENS160::temperaturaExterior(float temp)
{
  ...
  writeRegister(ENS160_TEMP_IN, tempUnion.bytes[0]);                    // Byte menos significativo
  writeRegister(ENS160_TEMP_IN + 1, tempUnion.bytes[1]);                // Byte más significativo
...
void L_ENS160::humedadExterior(float hum)
{
  ...
  writeRegister(ENS160_RH_IN, humUnion.bytes[0]);           // Byte menos significativo
  writeRegister(ENS160_RH_IN + 1, humUnion.bytes[1]);       // Byte más significativo
